...
//!
//! Each operation accepts the pre-computed Python hash plus a fallible eq closure, so unhashable
//! keys and exceptions raised by `__eq__` propagate as `PyResult` instead of panicking.
//!
//! The slab is a fixed-size `Box<[MaybeUninit<Node>]>` allocated up-front; eviction recycles slots
//! through a `Vec<u32>` LIFO. The `hashbrown::HashTable` is sized once with `with_capacity(maxsize)`
//! so it never resizes, which would otherwise call back into Python via the comparator. LRU order
//! is a doubly-linked list embedded in the slab; `INVALID` (`u32::MAX`) is the sentinel for both
//! ends of the list and the free chain.
...
/// Maximum value of `maxsize` accepted by [`Store::new`]. The slab uses `u32` indices so each
/// node packs into 32 bytes; `INVALID` (`u32::MAX`) is reserved as the link sentinel, leaving
/// `u32::MAX - 1` as the largest representable real index.
...
/// Python objects displaced by a successful [`Store::put`]. The caller must drop these *outside*
/// the store's lock: `Py_DECREF` can run arbitrary finalizers, and a finalizer that re-enters the
/// same cache would self-deadlock on the non-reentrant mutex.
...
impl Store {
    pub fn new(maxsize: NonZeroUsize) -> Self {
        ...
        // Reverse-ordered fill so the LIFO pops 0, 1, 2, ... on the initial insert sequence,
        // giving contiguous slab access on a fresh cache.
    ...
    /// Insert or update. Evicts the LRU entry when a fresh insert would exceed `maxsize`.
    pub fn put<F>(
        &mut self,
        hash: Py_hash_t,
        key: Py<PyAny>,
        ...
        F: FnMut(&Py<PyAny>) -> PyResult<bool>,
    {
        if let Some(idx) = self.find(hash, eq)? {
            ...
            self.move_to_head(idx);
            return Ok(Displaced { replaced: Some(replaced), evicted: None });
        }
        ...
        Ok(Displaced { replaced: None, evicted })
    }
    ...
    pub fn clear(&mut self, py: Python<'_>) {
        ...
        self.table.clear();
        // Walk LRU -> MRU via `prev`; `next` points toward the LRU end and would terminate
        ...
        while idx != INVALID {
            ...
            let prev = node.prev;
            // Drop key/value while attached so the refcount decrements run inline.
            drop_attached(py, node.key);
            drop_attached(py, node.value);
            self.free.push(idx);
            idx = prev;
        }
    }
    ...
    pub const fn iter_keys(&self) -> StoreIter<'_> {
        StoreIter { store: self, idx: self.tail, remaining: self.len() }
    }
    ...
    /// Remove an occupied slot from both the table and the LRU chain, returning its `(key, value)`.
    ...
    fn move_to_head(&mut self, idx: u32) {
        ...
        // SAFETY: `idx` is occupied; we are updating its prev/next to become the new head.
        unsafe {
...
fn drop_attached(_py: Python<'_>, obj: Py<PyAny>) {
    drop(obj);
}
