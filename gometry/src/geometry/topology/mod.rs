...
/// evaluates `orient2d(a, b, p)` per lane and trusts it only past Shewchuk's
/// A-stage filter: straddling lanes with `|det| <= errbound * (|t1| + |t2|)`
...
fn ray_crossing_lanes(
    ...
    let trusted = det
        .abs()
        .simd_gt((t1.abs() + t2.abs()) * ReduceSimd::splat(CCW_ERRBOUND_A));
...
mod tests {
    ...
    fn bench_ring(n: usize) -> Vec<XY> {
        let per_side = n / 4;
        let mut ring = Vec::with_capacity(n);
        for i in 0..per_side {
            let t = i as f64 / per_side as f64;
            ring.push(XY::new(-100.0 + 200.0 * t, -100.0));
        }
        for i in 0..per_side {
            let t = i as f64 / per_side as f64;
            ring.push(XY::new(100.0, -100.0 + 200.0 * t));
        }
        for i in 0..per_side {
            let t = i as f64 / per_side as f64;
            ring.push(XY::new(100.0 - 200.0 * t, 100.0));
    ...
    #[test]
    fn pip_scalar_bench() {
        let ring = bench_ring(7);
        let probe = XY::new(0.0, 0.0);
        assert!(ring_contains_interior_open_scalar(&ring, probe));
        let iters = 50_000;
        let start = std::time::Instant::now();
        for _ in 0..iters {
            std::hint::black_box(ring_contains_interior_open_scalar(&ring, probe));
        }
        let per_op_us = start.elapsed().as_secs_f64() * 1e6 / f64::from(iters);
        println!("pip_scalar_bench: {per_op_us:.3} us/op");
    }

    #[test]
    fn pip_open_ring_bench() {
        let ring = bench_ring(20_000);
        let probe = XY::new(0.0, 0.0);
        assert!(division_reference(&ring, probe));
        assert!(ring_contains_interior_open(&ring, probe));
        let warmup = 50;
        let iters = 500;
        for _ in 0..warmup {
            std::hint::black_box(ring_contains_interior_open(&ring, probe));
        }
        let start = std::time::Instant::now();
        for _ in 0..iters {
            std::hint::black_box(ring_contains_interior_open(&ring, probe));
        ...
        let per_op_us = elapsed.as_secs_f64() * 1e6 / f64::from(iters);
        println!("pip_open_ring_bench: {per_op_us:.1} us/op");
    }
}
