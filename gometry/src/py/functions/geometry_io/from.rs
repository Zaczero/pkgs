...
pub(crate) fn from_wkt(
    ...
    // Plain WKT carries no CRS; `crs` fills that gap. When the text is EWKT
    // its SRID is authoritative — but contradicting an *explicit* crs
    // argument is a caller mistake, not a precedence question: fail fast.
    ...
    let parse = |text: &str| -> PyResult<PyGeometry> {
        ...
        let crs = embedded.or_else(|| fallback.clone());
    ...
    // Bulk lane: an array carries ONE frame, so EWKT SRIDs must all agree —
    // a divergent row raises the same SharedCrs error `Frame::resolve_items`
    // raised. Valid inputs therefore always stream straight into packed
    // storage; no per-row geometry handles, no per-row frame column.
    let fallback_for_rows = fallback.clone();
    let array = stream_bulk(
        iter,
        fallback,
        parsed_epoch,
        Some("WKT import"),
        move |item, rows| {
            let text = item
                ...
                .map_err(|_| error())?
                .to_cow()?;
            let (body, embedded) = split_ewkt_srid(text.as_ref())?;
            guard_embedded_crs_conflict(
                embedded.as_deref(),
                fallback_for_rows.as_deref(),
                "EWKT SRID",
            )?;
            rows.try_push(io::parse_wkt(body)?)?;
            Ok(StreamedRow::Present(embedded))
        },
    )?;
    Ok(array.into_pyobject(py)?.unbind().into())
}

/// The shared row frame learned while streaming a bulk parse.
///
/// Also reused by mixed-array pickle reconstruction so an untrusted
/// reconstructor cannot apply a laxer CRS contract than bulk `from_wkb`.
...
pub(crate) fn stream_bulk<'py>(
    ...
    for (row, item) in iter.enumerate() {
        ...
        match parse_row(&item, &mut rows).map_err(|err| crate::note_array_row(err, row))? {
            ...
            StreamedRow::Missing => crate::try_push(&mut missing_rows, row)?,
        }
    }
    ...
    let present = rows.finish(Frame::new(crs, epoch)?);
...
/// An embedded SRID and an explicit ``crs=`` argument must agree: silently
/// preferring either side hides a caller mistake.
...
/// returning the WKT body and the embedded CRS.
pub(crate) fn split_ewkt_srid(text: &str) -> PyResult<(&str, Option<Crs>)> {
    ...
    // SRID 0 is PostGIS "unknown" → CRS-free; nonzero codes go through the
    // same PROJ-backed canonical CRS parser as every other CRS ingress.
    Ok((body, io::crs_from_srid(code)?))
}
...
pub(crate) fn from_wkb(
    ...
    // Plain WKB carries no CRS; `crs` fills that gap. EWKB embeds an SRID —
    // contradicting an explicit crs argument is a caller mistake: fail fast.
    let fallback = parse_crs(crs)?;
    let parsed_epoch = coordinate_epoch_option("epoch", epoch)?;
    let attach = |mut geometry: PyGeometry| -> PyResult<PyGeometry> {
        ...
        if geometry.crs_ref().is_none() {
            geometry.set_crs_keep_epoch(fallback.clone());
        }
        guard_epoch_frame(parsed_epoch, geometry.crs_ref())?;
        geometry.set_epoch_keep_crs(parsed_epoch);
        Ok(geometry)
    };
    // Retain the first buffer acquisition and parse it — do not probe-discard-
    // reacquire. TypeError means "not a one-byte buffer"; other errors are
    // genuine WKB failures and must not fall through to the iterable lane.
    match parse_wkb_payload_bytes(data, parse_wkb_geometry) {
        Ok(geometry) => {
            return Ok(Typed(attach(geometry)?).into_pyobject(py)?.unbind());
        },
        Err(err) if err.is_instance_of::<PyTypeError>(py) => {},
        Err(err) => return Err(err),
    }
    if let Ok(values) = data.try_iter() {
        // Bulk lane: an array carries ONE frame, so EWKB SRIDs must all agree
        // (see `check_shared_row_crs`). Valid inputs always stream straight
        // into packed storage — no per-row geometry handles.
        let fallback_for_rows = fallback.clone();
        let array = stream_bulk(
            values,
            fallback,
            ...
            Some("WKB import"),
            move |data, rows| {
                let parsed = parse_wkb_payload(data)?;
                guard_embedded_crs_conflict(
                    parsed.crs.as_deref(),
                    fallback_for_rows.as_deref(),
                    "EWKB SRID",
                ...
                Ok(StreamedRow::Present(parsed.crs.map(crate::crs_arc)))
            },
        )?;
...
/// Parse one WKB payload (bytes or any buffer exporter) to its raw
/// `WkbGeometry` — the bulk lane's per-row parse, no geometry handle.
...
/// `from_geojson`'s CRS argument: `geojson` is WGS84 by specification (RFC
/// 7946), so the omitted default declares ``EPSG:4326``; explicit ``None``
/// opts into a CRS-free geometry.
pub(crate) enum GeoJsonCrs {
    Wgs84,
    Resolved(Option<Crs>),
}
...
impl<'a, 'py> FromPyObject<'a, 'py> for GeoJsonCrs {
    type Error = PyErr;
...
#[pyfunction]
#[pyo3(
    signature = (data, *, crs = GeoJsonCrs::Wgs84, epoch = None),
    text_signature = "(data, *, crs=4326, epoch=None)"
)]
pub(crate) fn from_geojson(
    ...
    if let Ok(text) = data.cast::<pyo3::types::PyString>() {
        ...
        // Nested legacy ``crs`` is dropped by the typed text decoder — walk the
        // raw JSON once so Feature/geometry/GC-child declarations reconcile
        // through the single owner exactly like the mapping/bytes path.
        let raw: Value = serde_json::from_str(text.as_ref())
            .map_err(|error| io::IoError::geojson(error.to_string()))?;
        let mut declarations = Vec::new();
        io::collect_geojson_legacy_crs(&raw, &mut declarations);
        ...
        return match parsed.input {
            ...
            io::GeoJsonInput::FeatureCollection(features) => {
                ...
                let present = PyGeometryArray::from_shapes(shapes, frame);
                Ok(if let Some(mask) = mask {
                    PyGeometryArray::scatter_present_rows(&present, mask)
                } else {
                    present
                }
                .into_pyobject(py)?
    ...
    if let Some(features) = geojson_feature_collection(&parsed)? {
        ...
        let present = PyGeometryArray::from_shapes(shapes, frame);
        return Ok((if let Some(mask) = mask {
            PyGeometryArray::scatter_present_rows(&present, mask)
        } else {
            present
        })
...
#[pyfunction]
#[pyo3(
    signature = (features, *, crs = GeoJsonCrs::Wgs84, epoch = None),
    text_signature = "(features, *, crs=4326, epoch=None)"
)]
///
/// Examples
/// --------
...
fn build_features(
    ...
    for (row_index, row) in rows.into_iter().enumerate() {
        ...
        properties.push(match row.properties {
            FeatureSideData::Json(value) => feature_properties_py(py, value.as_ref())?,
            FeatureSideData::Python(value) => value,
        });
        ids.push(match row.id {
            FeatureSideData::Json(value) => feature_id_py(py, value.as_ref())?,
            FeatureSideData::Python(value) => value,
        });
    }
    let present = PyGeometryArray::from_shapes(shapes, Frame::new(crs, epoch)?);
    let geometries =
        if let Some(mask) = crate::array::sparse_missing_mask(row_count, &missing_rows) {
            PyGeometryArray::scatter_present_rows(&present, mask)
        } else {
...
/// Split parsed features into dense present shapes plus their missing-row mask.
...
fn feature_properties_py(py: Python<'_>, value: Option<&Value>) -> PyResult<Py<PyAny>> {
    match value {
        None => Ok(PyDict::new(py).unbind().into()),
        Some(Value::Null) => Ok(py.None()),
        Some(value @ Value::Object(_)) => json_to_py(py, value),
...
fn feature_id_py(py: Python<'_>, value: Option<&Value>) -> PyResult<Py<PyAny>> {
    match value {
        None | Some(Value::Null) => Ok(py.None()),
        Some(value @ (Value::String(_) | Value::Number(_))) => json_to_py(py, value),
        Some(_) => Err(feature_parse_error(
            "feature id must be a string or finite number",
        )),
    }
...
fn feature_parse_error(message: &'static str) -> PyErr {
    crate::py::errors::parse_error(message, crate::py::errors::ParseFormat::GeoJson)
}
...
fn feature_value_shapes_and_missing(
    ...
    for (row, feature) in features.iter().enumerate() {
        if let Some(shape) = geojson_feature_collection_member(feature)? {
            shapes.push(shape);
        } else {
            missing_rows.push(row);
        }
