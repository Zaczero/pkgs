...
//! (`crate::py::functions::predicate`), and exact refinement runs through the shared
...
use rstar::{AABB, Envelope, ParentNode, PointDistance, RTree, RTreeNode, RTreeObject};
...
use crate::{
    ...
    PointRows, Predicate, PyGeometry, PyGeometryArray, ShapeRow, bounds_envelope,
    crossing_index_bounds, crossing_index_envelope, crs, crs_label, epoch_label, exact_geometry,
    exact_geometry_array, geometry_items, global_geographic_candidate_envelope,
    pair_distance_resolved, pair_dwithin_resolved, point_from_bounds, resolve_metric,
...
pub(crate) struct PreparedIndexQuery {
    pub(crate) predicate: Option<IndexPredicate>,
    pub(crate) distance: Option<NonNegative>,
    pub(crate) metric: Option<crs::MetricModel>,
    pub(crate) resolved: Option<crs::ResolvedMetric>,
}
...
impl PreparedIndexQuery {
    ...
    fn for_frame(
        ...
        let metric = distance
            .map(|_| resolve_metric(metric_crs, unit, "spatial index distance query"))
            .transpose()?;
        let resolved = match predicate {
            Some(IndexPredicate::Dwithin) => metric
                .as_ref()
                .map(crs::ResolvedMetric::from_model)
                .transpose()?,
            Some(IndexPredicate::Topological(_)) | None => None,
        };
        Ok(Self {
            predicate,
            distance,
    ...
    pub(crate) fn row(&self, index: &PySpatialIndex, row: ShapeRow<'_>) -> PreparedIndexRow {
        let pruner_point = match row {
            ShapeRow::Point(point) => Some(point),
            _ => None,
        };
        let cap = row.with_shape(|shape| {
            query_geodesic_cap(pruner_point, shape, self.distance, self.metric.as_ref())
        });
        let bounds = row.quick_bounds();
        let bounds = if index.geographic() && row.with_shape(Shape::crosses_antimeridian) {
            bounds.map(|bounds| row.with_shape(|shape| crossing_index_bounds(shape, bounds)))
        } else {
            bounds
        };
        PreparedIndexRow {
    ...
    pub(crate) fn candidate_step(&self, index: &PySpatialIndex, query: Option<&Shape>) -> String {
        match (self.distance, self.metric.as_ref()) {
            (Some(distance), Some(metric)) => {
            ...
            _ => match self.predicate {
                ...
                _ => "bounds envelope candidate filter".to_owned(),
            },
        }
    }

    pub(crate) fn refine_step(&self) -> Option<String> {
        match self.predicate? {
            IndexPredicate::Dwithin => Some(format!(
                "exact {} distance refine within {}",
                self.metric
                    .as_ref()
                    .expect("dwithin resolves metric")
                    .explain_label(),
                self.distance.expect("dwithin requires distance").get(),
            )),
            IndexPredicate::Topological(predicate) => {
                Some(format!("exact {} predicate refine", predicate.token()))
            },
        }
    }
}
...
/// units when CRS-free.
#[pyclass(name = "SpatialIndex", module = "gometry", skip_from_py_object)]
#[derive(Debug)]
...
type PackedRows = Option<Arc<GeometryArrayStorage>>;
...
pub(crate) struct IndexRows {
    packed: PackedRows,
    packed_frame_caches: Option<crate::array::FrameCacheRows>,
...
impl IndexRows {
    fn len(&self) -> usize {
        let len = self.packed.as_ref().map_or(0, |rows| rows.len()) + self.boxed.len();
    ...
    fn get(&self, handle: usize) -> Option<ShapeRow<'_>> {
        let packed_len = self.packed.as_ref().map_or(0, |rows| rows.len());
        if handle < packed_len {
            return Some(self.packed.as_ref().expect("packed handle").row(handle));
        }
        self.boxed
    ...
    fn row(&self, handle: usize) -> ShapeRow<'_> {
        self.get(handle).expect("live index handle")
    }
...
impl HeapSize for IndexRows {
    fn heap_bytes(&self) -> usize {
        self.packed
            .as_ref()
            .map_or(0, |rows| rows.logical_heap_bytes())
...
/// The query side of the geodesic cap bound: points cap with zero reach
/// for free; non-point shapes pay one metric pass — only when a geodesic
/// distance query will actually consult the row cap table.
pub(crate) fn query_geodesic_cap(
    pruner_point: Option<Point>,
    shape: &Shape,
    distance: Option<NonNegative>,
    distance_model: Option<&crs::MetricModel>,
) -> Option<(Point, f64)> {
    if let Some(point) = pruner_point {
        return Some((point, 0.0));
    }
    distance?;
    let crs::MetricModel::Geodesic(crs) = distance_model? else {
        return None;
    };
    crs::with_ellipsoid_metric(crs, &[shape], |ellipsoid| Ok(shape.geodesic_cap(ellipsoid)))
        .ok()
        .flatten()
}
