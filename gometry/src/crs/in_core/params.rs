...
use super::super::{CrsCoordinateOperationInfo, DEGREE_TO_RADIAN, OperationParameterInfo};
use super::adjlon;
use super::kernel::{DEG_TO_RAD, ProjectionKernel, RAD_TO_DEG};
...
impl<K: ProjectionKernel> Projected<K> {
    pub(super) fn forward_xy(&self, lon_deg: f64, lat_deg: f64) -> Result<(f64, f64)> {
        ...
        Ok((x + self.frame.x_0, y + self.frame.y_0))
    }

    pub(super) fn inverse_xy(&self, x: f64, y: f64) -> Result<(f64, f64)> {
        let (lam_rel, lat_rad) =
            K::inverse_unframed(&self.setup, x - self.frame.x_0, y - self.frame.y_0)?;
