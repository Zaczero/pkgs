...
// Portable cube root for the astroid seed. Rust's scalar `f64::cbrt` links a
// runtime FMA/FMA4 dispatcher on x86-64-v2. A `powf` seed followed by two
// Newton refinements is amply accurate for this seed (the enclosing astroid
// solution performs its own stable reconstruction) and has invariant codegen
// across the supported baseline.
fn cbrt_unfused(value: f64) -> f64 {
    if value == 0.0 || !value.is_finite() {
        return value;
    }
    let magnitude = value.abs();
    let mut root = magnitude.powf(1.0 / 3.0);
    for _ in 0..2 {
...
mod tests {
    ...
    #[test]
    fn test_cbrt_unfused_special_values_and_cubes() {
        assert_eq!(cbrt_unfused(0.0).to_bits(), 0.0_f64.to_bits());
        assert_eq!(cbrt_unfused(-0.0).to_bits(), (-0.0_f64).to_bits());
        assert_eq!(cbrt_unfused(27.0), 3.0);
        assert_eq!(cbrt_unfused(-125.0), -5.0);
        assert!(cbrt_unfused(f64::NAN).is_nan());
        assert_eq!(cbrt_unfused(f64::INFINITY), f64::INFINITY);
    }

    #[test]
    fn test_sin_cos_series() {
        assert_eq!(
            sin_cos_series(false, -0.8928657853278468, 0.45032287238256896, &[
                0.6660771734724675,
        ...
        assert_eq!(
            sin_cos_series(true, 0.2969032234925426, 0.9549075745221299, &[
                0.0,
        ...
        assert_eq!(
            sin_cos_series(true, -0.8928657853278468, 0.45032287238256896, &[
                0.0,
                -0.0003561309485314716,
                -3.170731714689771e-08,
                -7.527972480734327e-12,
        ...
        assert_eq!(
            sin_cos_series(true, -0.024679833885152578, 0.9996954065111039, &[
                0.0,
