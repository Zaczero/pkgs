...
def test_f3_nested_mapping_proxy_and_keys_duck() -> None:
    ...
    assert gm.from_geojson(
        {"type": "Feature", "geometry": proxy_geom, "properties": {}}
    ).to_wkt() == "POINT (3 4)"
    assert gm.from_geojson(
        ...
        [
            {
                "type": "Feature",
                "geometry": collections.UserDict(
                    {"type": "Point", "coordinates": [13.0, 14.0]}
                ),
                "properties": {"k": 1},
            }
        ]
...
# ---------------------------------------------------------------------------
# F2 — Python twin: legacy getitem keys + lying short __len__
# ---------------------------------------------------------------------------
...
def test_f2_mapping_duck_legacy_getitem_keys_features() -> None:
    ...
    features = gm.Features(
        gm.GeometryArray([gm.Point(0, 0)]), properties=MappingDuck()
    )
    assert features.properties[0]["name"] == "ok"


def test_f2_lying_short_len_rejects() -> None:
    class ShortLen:
        def __len__(self):
            return 1
    ...
    with pytest.raises(gm.GeometryError, match="reports length"):
        gm.Features(gm.GeometryArray([gm.Point(0, 0)]), properties=ShortLen())
...
def test_d13_from_geoparquet_honest_columns_selection() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _geoms, attrs2 = gm.from_geoparquet(
            path, columns=(c for c in ["id", "label"])
        )
