    ...
    M: PerMessageDeflateMode,
{
    ...
    let result: Result<(), H2CornError> = 'session: loop {
        if let Some(outbound_event) = outbound
            .running_app
            .send_buffer
            .take_ready()
            ...
            .transpose()?
        {
            let control = process_outbound_batch::<T, M>(
                transport,
                Some(outbound_event),
                &mut deflater,
                &mut outbound,
            ...
            .await?;
            if let ControlFlow::Break(result) = control {
        ...
        let ping_sleep = async {
            if let Some(deadline) = next_ping {
        ...
        tokio::select! {
            result = &mut outbound.running_app.app_task, if !app_finished => {
                result??;
                app_finished = true;
                if !outbound.state.close_started() {
                    outbound.state.queue_close_if_open(close_code::NORMAL, "")?;
                }
                break 'session Ok(());
            }
            ...
            _ = ping_sleep, if next_ping.is_some() => {
                send_encoded_frame(transport, 0x9, &[], FrameFlushMode::Immediate, false).await?;
                next_ping = config
                    .ping_interval
                    .map(|interval| TokioInstant::now() + interval);
                if let Some(timeout_duration) = config.ping_timeout {
                    pong_deadline = Some(TokioInstant::now() + timeout_duration);
                }
            },
            _ = pong_sleep, if pong_deadline.is_some() => {
            ...
            changed = shutdown.changed() => {
                ...
                if let Some(kind) = shutdown_kind {
                    initiate_server_shutdown(&recv_tx, &mut outbound, kind).await?;
                    break 'session Ok(());
                }
            },
            read = transport.read_into_codec(&mut codec) => {
                if let ControlFlow::Break(result) = handle_transport_read::<T, M>(
                    transport,
                    read?,
                    &mut codec,
                    &mut inflater,
    ...
    M: PerMessageDeflateMode,
{
    ...
    let (payload, compressed) = match compressed_payload.as_deref() {
